...
export default function UserProfile() {
  ...
  const [profile, setProfile] = useState(() => {
    try { return JSON.parse(localStorage.getItem('user_profile') || 'null') || { name: '', email: '' } } catch { return { name: '', email: '' } }
  })

  const save = () => {
    try { localStorage.setItem('user_profile', JSON.stringify(profile)) } catch { /* no-op */ }
    notify({ title: 'Profile saved', description: 'Your profile has been updated', variant: 'success' })
  }
